...
void variant_fraction_partition(int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q, VV& gegen, V& gegen_int, V& vf, ofstream &f)
{
  ...
  if (h == 0)
    {
      for (int s=1; s<=FRACTIONS; ++s)
        {
          vf[s] = variant_fraction(s, 0, q, n_q, t_q, t_q_h, beta_tilda_q, gegen, gegen_int);
          if (s < FRACTIONS)
            vf[s] *= interval;
        }
  ...
  else
    {
      for (int s=0; s<=FRACTIONS; ++s)
        {
          vf[s] = variant_fraction(s, 1, q, n_q, t_q, t_q_h, beta_tilda_q, gegen, gegen_int);
          if (0 < s && s < FRACTIONS)
            vf[s] *= interval;
        }
      ...
      for (int _t = 1; _t <=T_MAX; ++_t)
        {
          ...
          string outf = "./variant_fraction_discrete_all_data/variant_fraction_discrete_all_h0_betatilda" + to_string(beta_tilda_disc) + "_t" + to_string(_t);
          ofstream ff(outf);
          ...
          for (int _t_h = 0; _t_h<T_H_MAX; ++_t_h)
            {
              double t_q_h = t_q * _t_h / T_H_MAX;
              
              string outf2 = "./variant_fraction_discrete_all_data/variant_fraction_discrete_all_h1_betatilda" + to_string(beta_tilda_disc) + "_t" + to_string(_t) + "_t_h" + to_string(_t_h);
              ofstream ff2(outf2);
          
              variant_fraction_partition(1, 4, 0.1, t_q, t_q_h, beta_tilda_q, gegen, gegen_int, vf_h1, f);
              write_vector_vert(ff2, vf_h1, FRACTIONS);
              ff2.close();
            }
