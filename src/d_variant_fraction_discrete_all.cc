  ...
  for (int beta_tilda_disc=1; beta_tilda_disc<=BETA_TILDA_MAX; ++beta_tilda_disc)
    {
      ...
      for (int _t=1; _t<=T_MAX; ++_t)
        {
          string outf = "d_t_variant_fraction_discrete_all_h0_betatilda" + to_string(beta_tilda_disc) + "_t" + to_string(_t);
          ofstream ff(outf);
      ...
      cout << "h = 0" << endl;
      f << "h = 0" << endl;
      variant_fraction_partition(0, 4, 0.1, t_q, t_q/5.0, beta_tilda_q, gegen, gegen_int, vf_h0, vf_partition_h0, f);
      // write_vector(f, vf_h0, FRACTIONS+1);
      d_variant_fraction_partition(d_t_variant_fraction, 0, 4, 0.1, t_q, t_q/5.0, beta_tilda_q, gegen, gegen_int, vf_h0, dvf_h0, vf_partition_h0, f);
      f << "d_t_result: ";
