...
void write_int_vector(vector<int> &a, int n)
{
  cout << scientific;
  for (int i=0; i<n; ++i)
    cout << a[i] << " ";
...
double variant_fraction_sub(int s, int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q)
{
          ...
          for (int i=GEGEN_MAX; i>0; --i)
            {
              ...
              double a = 
                ...
                (exp(g) * (gsl_sf_expint_E1(g) - gsl_sf_expint_E1(g*Nnq)) * g  - 1.0 + 1.0/g + exp(-g * (Nnq - 1.0))/Nnq);
              // cout << a << endl;
              if (isnan(a))
                cout << "a is nan" << endl;
...
double variant_fraction_sub_normalized(int s, int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q)
{
  V vf(FRACTIONS+1, 0);
  double partition = 0;
  ...
  if (h == 0)
    {
      for (int x=1; x<=FRACTIONS; ++x)
        {
          vf[x] = variant_fraction_sub(x, 0, q, n_q, t_q, t_q_h, beta_tilda_q);
        }
  ...
  else
    {
      for (int x=0; x<=FRACTIONS; ++x)
        {
          vf[x] = variant_fraction_sub(x, 1, q, n_q, t_q, t_q_h, beta_tilda_q);
        }
...
double func_t(double t_q, void *params)
{
  ...
  return variant_fraction_sub_normalized(p->s, p->h, p->q, p->n_q, t_q, p->t_q_h, p->beta_tilda_q);
}

void d_t_variant_fraction_sub_normalized(int s, int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q, double* result, double* abserr)
{
  gsl_function F;
...
double func_t_h(double t_q_h, void *params)
{
  ...
  return variant_fraction_sub_normalized(p->s, p->h, p->q, p->n_q, p->t_q, t_q_h, p->beta_tilda_q);
}

void d_t_h_variant_fraction_sub_normalized(int s, int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q, double* result, double* abserr)
{
  gsl_function F;
...
double func_n(double n_q, void *params)
{
  ...
  return variant_fraction_sub_normalized(p->s, p->h, p->q, n_q, p->t_q, p->t_q_h, p->beta_tilda_q);
}

void d_n_variant_fraction_sub_normalized(int s, int h, int q, double n_q, double t_q, double t_q_h, double beta_tilda_q, double* result, double* abserr)
{
  gsl_function F;
...
void write_params(std::ostream& f, params pa)
{
  f << "u" << endl;
  for (int i=2; i<=MAX_SUBTYPE; ++i)
    {
  ...
  for (int i=0; i<=MAX_SUBTYPE; ++i)
    {
      ...
      if (isnan(n[i]))
        cout << "n[" << i << "] is nan" << endl;

      if (isnan(st.l[i]))
        cout << "st.l[" << i << "] is nan" << endl;

      if (isnan(st.r[i]))
        cout << "st.r[" << i << "] is nan" << endl;
  ...
  if (numerator > denominator)
    {
      ...
      cout << "n : ";
      write_vector(n, MAX_SUBTYPE+1);
      cout << "st.l : ";
      write_int_vector(st.l, MAX_SUBTYPE+1);
      cout << "st.r : ";
      write_int_vector(st.r, MAX_SUBTYPE+1);
    }

  if (isnan(numerator/denominator))
    cout << "mu " << numerator << " " << denominator << endl;
...
double read_prob(READ re, state st, std::vector<double> &n, double epsilon)
{
  ...
  if (isnan(p))
    cout << re.first << " " << mu << " " << re.second << endl;
  
  // if (p)
  //   {
  //     if (p == GSL_EINVAL)
  //       cout << "GSL_EINVAL" << endl;
  //     else if (p == GSL_EDOM)
  //       cout << "GSL_EDOM" << endl;
  //   }
...
double variant_fraction(int s, int h, int i, double n_i, double t_i, double t_i_h, double beta_tilda)
{
  if (h == 0 && s == 0)
    return 0;

  return variant_fraction_sub_normalized(s, h, i, n_i, t_i, t_i_h, beta_tilda);
  // return n_i / x_i / beta_tilda;
}

double d_t_variant_fraction(int s, int h, int i, double n_i, double t_i, double t_i_h, double beta_tilda)
{
  if (h == 0 && s == 0)
    return 0;
  ...
  d_t_variant_fraction_sub_normalized(s, h, i, n_i, t_i, t_i_h, beta_tilda, &result, &abserr);
  
  return result;
  // return 0;
}

double d_t_h_variant_fraction(int s, int h, int i, double n_i, double t_i, double t_i_h, double beta_tilda)
{
  if (h == 0 && s == 0)
    return 0;
  ...
  d_t_h_variant_fraction_sub_normalized(s, h, i, n_i, t_i, t_i_h, beta_tilda, &result, &abserr);
  
  return result;
  // return 0;
}

double d_n_variant_fraction(int s, int h, int i, double n_i, double t_i, double t_i_h, double beta_tilda)
{
  if (h == 0 && s == 0)
    return 0;
  ...
  d_n_variant_fraction_sub_normalized(s, h, i, n_i, t_i, t_i_h, beta_tilda, &result, &abserr);
...
double lik_k(state* curr_st, params* pa_old, hyperparams hpa, READ re, V& n)
{
  double a = 0;
          ...
          for (int s=0; s<=FRACTIONS; ++s)
            {
              ...
              double vf = variant_fraction(curr_st->s, curr_st->h, curr_st->q, n[curr_st->q], calc_t(curr_st->q, pa_old->u), tq_h, pa_old->beta_tilda[curr_st->q]);
              if (isnan(vf))
                cout << "vf is nan!" << endl;

              double rp = read_prob(re, *curr_st, n, hpa.epsilon);
              if (isnan(rp))
                cout << "rp is nan!" << endl;
  ...
  if (isnan(a))
    cout << "lik is nan!" << endl;
  
  return a;
}

double lik_subtype(state* curr_st, params* pa_old, hyperparams hpa, READ re, int curr_subtype, V &n)
{
  if (curr_subtype > MAX_SUBTYPE)
    return lik_k(curr_st, pa_old, hpa, re, n);
      ...
      for (curr_st->r[curr_subtype] = 1; curr_st->r[curr_subtype] <= curr_st->l[curr_subtype]; ++curr_st->r[curr_subtype])
        {
          ...
          s += pa_old->kappa[curr_st->l[curr_subtype]][curr_st->r[curr_subtype]] * lik_subtype(curr_st, pa_old, hpa, re, curr_subtype + 1, n);
        }
      // cout << endl;
...
double llik(READS& res, params* pa, hyperparams hpa)
{
  write_params(cout, *pa);
  ...
  for (READS::iterator re = res.begin(); re != res.end(); ++re)
    {
      ...
      sum += log(lik_subtype(curr_st, pa, hpa, **re, 1, n));
      count++;
    }
  return sum;
}
...
double my_f(const gsl_vector *v, void *rsph_par)
{
  ...
  for (int i=2; i<=MAX_SUBTYPE; ++i)
    pa->u[i] = calc_sigmoid(gsl_vector_get(v, i-2));
  pa->beta[0][0] = calc_sigmoid(gsl_vector_get(v, MAX_SUBTYPE-1));

  for (int i=1; i<=NONLEAF; ++i)
    for (int j=0; j<MAX_CHILD; ++j)
      pa->beta[i][j] = calc_sigmoid(gsl_vector_get(v, MAX_SUBTYPE + MAX_CHILD*(i-1) + j));
  ...
  return -llik(p->res, pa, p->hpa);
}

double dui_lik_k(state* curr_st, params* pa_old, hyperparams hpa, READ re, V& n, int i)
{
  double a = 0;
      ...
      for (curr_st->h = 0; curr_st->h < MAX_H; ++curr_st->h)
        {
          ...
          double coef_t, coef_t_h;
          
          int i_parent = calc_parent(i);
          int qk_parent = curr_st->q;
          int qk_prev_parent = curr_st->q;

          int q_hk = first_inherit_child(curr_st->q, curr_st->h);

          double tq = calc_t(curr_st->q, pa_old->u);
          double tq_h = -1;
          if (curr_st->h != 0)
            {
              tq_h = calc_t(q_hk, pa_old->u);
            }
          ...
          for (int s=0; s<=FRACTIONS; ++s)
            {
              ...
              c += (coef_t * d_t_variant_fraction(curr_st->s, curr_st->h, curr_st->q, n[curr_st->q], calc_t(curr_st->q, pa_old->u), tq_h, pa_old->beta_tilda[curr_st->q])
                    + coef_t_h * d_t_h_variant_fraction(curr_st->s, curr_st->h, curr_st->q, n[curr_st->q], calc_t(curr_st->q, pa_old->u), tq_h, pa_old->beta_tilda[curr_st->q])) * read_prob(re, *curr_st, n, hpa.epsilon);
            }
          b += pa_old->omega[curr_st->h] * c;
        }
      a += pa_old->xi[curr_st->q] * b;
...
double dui_lik_subtype(state* curr_st, params* pa_old, hyperparams hpa, READ re, int curr_subtype, V &n, int i)
{
  if (curr_subtype > MAX_SUBTYPE)
    return dui_lik_k(curr_st, pa_old, hpa, re, n, i);
  ...
  for (curr_st->l[curr_subtype] = 1; curr_st->l[curr_subtype] <= MAX_COPY; ++curr_st->l[curr_subtype])
    {
      ...
      for (curr_st->r[curr_subtype] = 1; curr_st->r[curr_subtype] <= curr_st->l[curr_subtype]; ++curr_st->r[curr_subtype])
        {
          // cout << "curr_st->r[" << curr_subtype << "] =" << curr_st->r[curr_subtype] << endl;
          s += pa_old->kappa[curr_st->l[curr_subtype]][curr_st->r[curr_subtype]] * dui_lik_subtype(curr_st, pa_old, hpa, re, curr_subtype + 1, n, i);
        }
      // cout << endl;
...
double dui_llik(READS& res, params* pa, hyperparams hpa, double llik, int i)
{
  state* curr_st = new state;
  init_state(curr_st);
  ...
  // for (READS::iterator re = res.begin(); re != res.end(); ++re)
  //   {
  //     a += log(dui_lik_subtype(curr_st, pa, hpa, **re, 1, n, i));
  //   }
  ...
  for (READS::iterator re = res.begin(); re != res.end(); ++re)
    {
      sum *= dui_lik_subtype(curr_st, pa, hpa, **re, 1, n, i);
    }
...
double dmu_dny(state st, std::vector<double> n, double epsilon, int y) // st is the state of y
{
  ...
  return (epsilon * st.l[y] + (1 - 2*epsilon) * ((double)st.s) / ((double)FRACTIONS) * st.r[y]) / denominator + st.l[y] * numerator / denominator / denominator;
}
...
double dbeta_lik_k(state* curr_st, params* pa, hyperparams hpa, READ re, V& n, int i, int j)
{
  double a = 0;
          ...
          for (int s=0; s<=FRACTIONS; ++s)
            {
              ...
              double factor = 0;
              double mu = calc_mu(*curr_st, n, hpa.epsilon);

              double rp = read_prob(re, *curr_st, n, hpa.epsilon);
              
              for (int y=i; y<=MAX_SUBTYPE; ++y)
                {
                  factor = n[y] * beta_factor(y, i, j, pa) * dmu_dny(*curr_st, n, hpa.epsilon, y) * dreadprob_dmu(re, mu);
                }

              c += d_n_variant_fraction(curr_st->s, curr_st->h, curr_st->q, n[curr_st->q], calc_t(curr_st->q, pa->u), tq_h, pa->beta_tilda[curr_st->q]) * rp
                + variant_fraction(curr_st->s, curr_st->h, curr_st->q, n[curr_st->q], calc_t(curr_st->q, pa->u), tq_h, pa->beta_tilda[curr_st->q]) * factor / rp;
            }
          b += pa->omega[curr_st->h] * c;
        }
      a += pa->xi[curr_st->q] * b;
    }
  return a;
...
double dbeta_lik_subtype(state* curr_st, params* pa, hyperparams hpa, READ re, int curr_subtype, V &n, int i, int j)
{
  if (curr_subtype > MAX_SUBTYPE)
    return dbeta_lik_k(curr_st, pa, hpa, re, n, i, j);
  ...
  for (curr_st->l[curr_subtype] = 1; curr_st->l[curr_subtype] <= MAX_COPY; ++curr_st->l[curr_subtype])
    {
      ...
      for (curr_st->r[curr_subtype] = 1; curr_st->r[curr_subtype] <= curr_st->l[curr_subtype]; ++curr_st->r[curr_subtype])
        {
          // cout << "curr_st->r[" << curr_subtype << "] =" << curr_st->r[curr_subtype] << endl;
          s += pa->kappa[curr_st->l[curr_subtype]][curr_st->r[curr_subtype]] * dbeta_lik_subtype(curr_st, pa, hpa, re, curr_subtype + 1, n, i, j);
        }
      // cout << endl;
      
      t += pa->pi[curr_st->l[curr_subtype]] * s;
    }
  return t;
}

double dbeta_llik(READS& res, params* pa, hyperparams hpa, double llik, int i, int j)
{
  state* curr_st = new state;
  init_state(curr_st);
  ...
  // for (READS::iterator re = res.begin(); re != res.end(); ++re)
  //   {
  //     a += log(dbeta_lik_subtype(curr_st, pa, hpa, **re, 1, n, i, j));
  //   }
  ...
  for (READS::iterator re = res.begin(); re != res.end(); ++re)
    {
      sum *= dbeta_lik_subtype(curr_st, pa, hpa, **re, 1, n, i, j);
    }
...
void my_df(const gsl_vector *v, void *rsph_par, gsl_vector *df)
{
  ...
  double llk = llik(p->res, pa, p->hpa);
  
  for (int i=2; i<=MAX_SUBTYPE; ++i)
    gsl_vector_set(df, i-2, -calc_dx_sigmoid(gsl_vector_get(v, i-2)) * dui_llik(p->res, pa, p->hpa, llk, i));
  gsl_vector_set(df, MAX_SUBTYPE-1, -calc_dx_sigmoid(gsl_vector_get(v, MAX_SUBTYPE-1)) * dbeta_llik(p->res, pa, p->hpa, llk, 0, 0));

  for (int i=1; i<=NONLEAF; ++i)
    for (int j=0; j<MAX_CHILD; ++j)
      gsl_vector_set(df, MAX_SUBTYPE + MAX_CHILD*(i-1) + j, -calc_dx_sigmoid(gsl_vector_get(v, MAX_SUBTYPE + MAX_CHILD*(i-1) + j)) * dbeta_llik((p->res), pa, p->hpa, llk, i, j));
}
...
void maximization(params* pa_old, params* pa_new, hyperparams hpa, READS res, gsl_vector* x, int count, gsl_multimin_function_fdf my_func) // needs init x0
{
  ...
  // if (count == 0)
  //   {
      cout << "maximization: count = 0" << endl;
      x = gsl_vector_alloc(2*MAX_SUBTYPE - 1);
      for (int i=0; i<2*MAX_SUBTYPE-1; ++i)
        gsl_vector_set(x, i, 0.5); // initial value sigmoid(0.5) ~ 0.622
    // }
  ...
  gsl_multimin_fdfminimizer_set(s, &my_func, x, 0.01, 1e-4); // this is minimizer, so multiply -!
  ...
  do
    {
      ...
      cout << "llik: " << -s->f << endl; // because function is multiplied by -

      for (int i=2; i<=MAX_SUBTYPE; ++i)
        pa_new->u[i] = calc_sigmoid(gsl_vector_get(s->x, i-2));
      pa_new->beta[0][0] = calc_sigmoid(gsl_vector_get(s->x, MAX_SUBTYPE-1));
      
      for (int i=1; i<=NONLEAF; ++i)
        for (int j=0; j<MAX_CHILD; ++j)
          pa_new->beta[i][j] = calc_sigmoid(gsl_vector_get(s->x, MAX_SUBTYPE + MAX_CHILD*(i-1) + j));
...
// {
//   if (strcmp(gsl_strerror(gsl_errno), "underflow") != 0)
//     {
//       cout << "err: " << gsl_strerror(gsl_errno) << endl;
//     }
// }
  ...
  for (int i=0; i<em_maxit; ++i)
    {
      ...
      maximization(pa_old, pa_new, *hpa, *res, x, i, my_func);
      
      write_params(outfile, *pa_new);
      
      if (check_converge(*pa_new, *pa_old, em_eps))
        break;
      *pa_old = *pa_new;
      // delete pa_new;
    }
