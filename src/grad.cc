...
struct ComputePdubeta {
  ...
  int operator()(const std::vector<double> x, double& fn, std::vector<double>& gr) { // x: reference?
    ...
    for (int i=2; i<=MAX_SUBTYPE; ++i)
      cerr << (gr[i-2] = -dllk_dui(_sts, _pa, _hpa, i)) << endl;
    
    cerr << (gr[MAX_SUBTYPE-1] = -dllik_dbeta(_res, _sts, _pa, _hpa, 0, 0)) << endl;
    
    for (int i=1; i<=NONLEAF; ++i)
      for (int j=0; j<MAX_CHILD; ++j)
        cerr << (gr[MAX_SUBTYPE + MAX_CHILD*(i-1) + j] = -dllik_dbeta(_res, _sts, _pa, _hpa, i, j)) << endl;
...
void maximization(params* pa_old, params* pa_new, hyperparams hpa, READS res, Lbfgsb* minimizer, int count) // needs init x0
{
  ...
  if (count > 0)
    ...
    {
      const vector<double> x0(2*MAX_SUBTYPE, 0.5);
      minimizer->minimize(x0, ComputePdubeta(res, sts, pa_new, hpa));
    }
  ...
  for (int i=0; i<em_maxit; ++i)
    {
      ...
      maximization(pa_old, pa_new, *hpa, *res, &minimizer, i); // res io is needed
